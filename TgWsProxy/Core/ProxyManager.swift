...
struct ProxyStats {
    ...
    var description: String {
        ...
        if cfWorker > 0 { parts.append("cfw:\(cfWorker)") }
...
class ProxyManager: ObservableObject {
    ...
    func getLogs() -> [String] {
        guard let ptr = GetLogs() else { return [] }
        let raw = String(cString: ptr)
        FreeString(ptr)
    ...
    private static func logLevel(for line: String) -> LogLevel {
        if line.hasPrefix("[ERROR] ") { return .error }
        if line.hasPrefix("[WARN] ") { return .warn }
        if line.hasPrefix("[DEBUG] ") { return .debug }
        return .info
    }
    ...
    private func parseStats(_ raw: String) -> ProxyStats {
        ...
        s.cfWorker = extractStat(raw, key: "cfw=") ?? 0
